...
class MainActivity: FlutterActivity() {
	...
	private var pendingEarthquakeParams: Map<String, Any>? = null
			...
			override fun onResume() {
				...
				if (flutterEngine != null) {
					...
					android.util.Log.w("DepremApp", "onResume: flutterEngine is null, params iletilemedi! Beklemeye alındı.")
					pendingEarthquakeParams = params
				}
				...
				if (params == null) {
					android.os.Handler().postDelayed({
						...
						// Sadece pendingEarthquakeParams'a ata, doğrudan flutterEngine'a erişme
						pendingEarthquakeParams = delayedParams
					}, 500)
				}
			}
	...
	private fun getEarthquakeParamsFromIntent(intent: Intent?): Map<String, Any>? {
		...
		android.util.Log.d("DepremApp", "getEarthquakeParamsFromIntent: intent action = " + intent.action)
		android.util.Log.d("DepremApp", "getEarthquakeParamsFromIntent: intent data = " + intent.data)
		
		// Debug: Check what types the extras are
		intent.extras?.let { extras ->
			extras.keySet().forEach { key ->
				val value = extras.get(key)
				android.util.Log.d("DepremApp", "  Extra key=$key, value=$value, type=${value?.javaClass?.simpleName}")
			}
		}
		
		if (intent.hasExtra("magnitude") || intent.hasExtra("location") || intent.hasExtra("distance")) {
			val params = mutableMapOf<String, Any>()
			// Try to get magnitude as Double, if it fails try as String
			val rawMagnitude = intent.extras?.get("magnitude")
			val magnitude = when (rawMagnitude) {
				is Double -> rawMagnitude
				is Float -> rawMagnitude.toDouble()
				is String -> rawMagnitude.toDoubleOrNull() ?: 0.0
				is Number -> rawMagnitude.toDouble()
				else -> intent.getDoubleExtra("magnitude", 0.0)
			}
			// Same for distance
			val rawDistance = intent.extras?.get("distance")
			val distance = when (rawDistance) {
				is Double -> rawDistance
				is Float -> rawDistance.toDouble()
				is String -> rawDistance.toDoubleOrNull() ?: 0.0
				is Number -> rawDistance.toDouble()
				else -> intent.getDoubleExtra("distance", 0.0)
			}
	...
	override fun configureFlutterEngine(flutterEngine: FlutterEngine) {
		...
		// Eğer bekleyen parametre varsa, Flutter hazır olunca gönder
		pendingEarthquakeParams?.let {
			android.util.Log.d("DepremApp", "configureFlutterEngine: pendingEarthquakeParams Flutter'a iletiliyor: $it")
			sendEarthquakeParamsToFlutter(it)
			pendingEarthquakeParams = null
		}
		...
		// Settings channel - Pil optimizasyonu ayarları
		MethodChannel(flutterEngine.dartExecutor.binaryMessenger, "deprem_app/settings").setMethodCallHandler { call, result ->
			if (call.method == "openBatterySettings") {
				try {
					...
					android.util.Log.e("DepremApp", "Pil ayarları açılamadı: ${e.message}")
	...
	private fun showEarthquakeAlertActivity(magnitude: Double, location: String, distance: Double) {
		val intent = Intent(this, Class.forName("com.fsapps.earthquake_line.EarthquakeAlertActivity"))
