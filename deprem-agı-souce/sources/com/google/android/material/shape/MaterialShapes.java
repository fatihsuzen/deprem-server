...
public final class MaterialShapes {
    ...
    @NonNull
    @RestrictTo({RestrictTo.Scope.LIBRARY_GROUP})
    static Matrix createRotationMatrix(float f5) {
    ...
    @NonNull
    @RestrictTo({RestrictTo.Scope.LIBRARY_GROUP})
    static Matrix createScaleMatrix(float f5, float f6) {
        Matrix matrix = new Matrix();
    ...
    @NonNull
    @RestrictTo({RestrictTo.Scope.LIBRARY_GROUP})
    static Matrix createSkewMatrix(float f5, float f6) {
        Matrix matrix = new Matrix();
        matrix.setSkew(f5, f6);
        return matrix;
    }

    @NonNull
    private static RoundedPolygon customPolygon(@NonNull List<VertexAndRounding> list, int i5, float f5, float f6, boolean z4) {
        ArrayList arrayList = new ArrayList();
        float f7 = f5;
        float f8 = f6;
        repeatAroundCenter(list, arrayList, i5, f7, f8, z4);
        return RoundedPolygonKt.RoundedPolygon(toVerticesXyArray(arrayList), CornerRounding.Unrounded, toRoundingsList(arrayList), f7, f8);
    }
    ...
    private static RoundedPolygon getArch() {
        ...
        return Shapes_androidKt.transformed(RoundedPolygonKt.RoundedPolygon(4, 1.0f, 0.0f, 0.0f, cornerRounding, Arrays.asList(new CornerRounding[]{cornerRounding2, cornerRounding2, cornerRounding3, cornerRounding3})), createRotationMatrix(-135.0f));
    }
    ...
    private static RoundedPolygon getSemiCircle() {
        ...
        return ShapesKt.rectangle(companion, 1.6f, 1.0f, cornerRounding, Arrays.asList(new CornerRounding[]{cornerRounding2, cornerRounding2, cornerRounding3, cornerRounding3}), 0.0f, 0.0f);
    }
    ...
    @NonNull
    @RestrictTo({RestrictTo.Scope.LIBRARY_GROUP})
    public static RoundedPolygon normalize(@NonNull RoundedPolygon roundedPolygon, boolean z4, @NonNull RectF rectF) {
    ...
    private static void repeatAroundCenter(@NonNull List<VertexAndRounding> list, @NonNull List<VertexAndRounding> list2, int i5, float f5, float f6, boolean z4) {
        boolean z5;
        int i6;
        float f7;
        list2.clear();
        toRadial(list, f5, f6);
        float f8 = (float) (6.283185307179586d / ((double) i5));
        if (z4) {
            int i7 = i5 * 2;
            float f9 = f8 / 2.0f;
            for (int i8 = 0; i8 < i7; i8++) {
                for (int i9 = 0; i9 < list.size(); i9++) {
                    ...
                    VertexAndRounding vertexAndRounding = list.get(i6);
                    if (i6 > 0 || !z5) {
                        float f10 = ((float) i8) * f9;
                        if (z5) {
                            f7 = (f9 - vertexAndRounding.vertex.x) + (list.get(0).vertex.x * 2.0f);
                        } else {
                            f7 = vertexAndRounding.vertex.x;
                        }
                        list2.add(new VertexAndRounding(new PointF(f10 + f7, vertexAndRounding.vertex.y), vertexAndRounding.rounding));
                    }
                }
            }
        } else {
            for (int i10 = 0; i10 < i5; i10++) {
                for (VertexAndRounding next : list) {
                    list2.add(new VertexAndRounding(new PointF((((float) i10) * f8) + next.vertex.x, next.vertex.y), next.rounding));
                }
            }
        }
        toCartesian(list2, f5, f6);
    }

    private static void toCartesian(@NonNull List<VertexAndRounding> list, float f5, float f6) {
        for (VertexAndRounding access$500 : list) {
            access$500.toCartesian(f5, f6);
        }
    }

    private static void toRadial(@NonNull List<VertexAndRounding> list, float f5, float f6) {
        for (VertexAndRounding access$400 : list) {
            access$400.toRadial(f5, f6);
        }
    }
    ...
    private static float[] toVerticesXyArray(@NonNull List<VertexAndRounding> list) {
        ...
        for (int i5 = 0; i5 < list.size(); i5++) {
            int i6 = i5 * 2;
            fArr[i6] = list.get(i5).vertex.x;
            fArr[i6 + 1] = list.get(i5).vertex.y;
        }
        return fArr;
    }
    ...
    static class VertexAndRounding {
        ...
        /* access modifiers changed from: private */
        public void toCartesian(float f5, float f6) {
            PointF pointF = this.vertex;
            float cos = (float) ((((double) pointF.y) * Math.cos((double) pointF.x)) + ((double) f5));
            PointF pointF2 = this.vertex;
            PointF pointF3 = this.vertex;
            pointF3.x = cos;
            pointF3.y = (float) ((((double) pointF2.y) * Math.sin((double) pointF2.x)) + ((double) f6));
        }

        /* access modifiers changed from: private */
        public void toRadial(float f5, float f6) {
            this.vertex.offset(-f5, -f6);
            PointF pointF = this.vertex;
            PointF pointF2 = this.vertex;
            float hypot = (float) Math.hypot((double) pointF2.x, (double) pointF2.y);
            PointF pointF3 = this.vertex;
            pointF3.x = (float) Math.atan2((double) pointF.y, (double) pointF.x);
            pointF3.y = hypot;
        }
    ...
    @NonNull
    private static RoundedPolygon getTriangle(float f5) {
        return Shapes_androidKt.transformed(getTriangle(), createRotationMatrix(f5));
    }

    @NonNull
    private static RoundedPolygon getOval(float f5) {
        return Shapes_androidKt.transformed(getOval(), createRotationMatrix(f5));
    }

    @NonNull
    private static RoundedPolygon getGem(float f5) {
        return Shapes_androidKt.transformed(getGem(), createRotationMatrix(f5));
    }

    @NonNull
    @RestrictTo({RestrictTo.Scope.LIBRARY_GROUP})
    public static RoundedPolygon normalize(@NonNull RoundedPolygon roundedPolygon, boolean z4) {
        return normalize(roundedPolygon, z4, new RectF(0.0f, 0.0f, 1.0f, 1.0f));
    }
}
