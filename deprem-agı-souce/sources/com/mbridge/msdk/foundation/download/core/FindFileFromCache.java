...
class FindFileFromCache implements IDownloadTask {
    ...
    private boolean checkDownloadReadyRate(long j5) {
        if (Utils.getDownloadRate(this._downloadModel.getTotalBytes(), j5) >= this._downloadMessage.getDownloadRate()) {
            return true;
        }
        return false;
    }
    ...
    private boolean checkUseFileResume() {
        boolean z4;
        try {
            z4 = Boolean.parseBoolean(this._request.get("do_us_fi_re", Boolean.toString(true)));
        ...
        builder.add(DownloadCommon.DOWNLOAD_REPORT_FIND_FILE_RESULT, DownloadCommon.DOWNLOAD_REPORT_FIND_FILE_RESULT_VALUE_C + z4);
        return z4;
    }
    ...
    private void resumeDownloadFromFile(DownloadResponse downloadResponse, long j5) {
        long j6 = j5;
        this._request.setTotalBytes(this._downloadModel.getTotalBytes());
        this._request.setDownloadedBytes(j6);
        GlobalComponent.getInstance().getDatabaseHelper().update(DownloadModel.create(this._downloadModel.getDownloadId(), this._downloadModel.getDownloadUrl(), this._downloadModel.getResourceUrl(), this._downloadModel.getEtag(), this._downloadModel.getSaveDirectorPath(), this._downloadModel.getSaveFileName(), this._downloadModel.getTotalBytes(), j6, this._downloadModel.getDownloadRate(), this._downloadModel.getUsageCounter(), this._downloadModel.getDownloadResourceType()), this._request.getCacheDirectoryPath());
        downloadResponse.setSuccessful(checkDownloadReadyRate(j6));
    }
