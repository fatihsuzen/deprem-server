...
class Huffman {
    ...
    private void addCode(int i5, int i6, byte b5) {
        Node node = new Node(i5, b5);
    ...
    public byte[] decode(byte[] bArr) {
        ...
        for (byte b6 : bArr) {
            ...
            i5 += 8;
            while (i5 >= 8) {
                node = node.children[(b5 >>> (i5 - 8)) & 255];
        ...
        while (i5 > 0) {
            Node node2 = node.children[(b5 << (8 - i5)) & 255];
    ...
    public int encodedLength(ByteString byteString) {
        ...
        for (int i5 = 0; i5 < byteString.size(); i5++) {
            j5 += (long) CODE_LENGTHS[byteString.getByte(i5) & 255];
        }
        return (int) ((j5 + 7) >> 3);
    }
