...
public final class LatmReader implements ElementaryStreamReader {
    ...
    private void parsePayloadMux(ParsableBitArray parsableBitArray, int i5) {
        int position = parsableBitArray.getPosition();
        if ((position & 7) == 0) {
            this.sampleDataBuffer.setPosition(position >> 3);
        } else {
            parsableBitArray.readBits(this.sampleDataBuffer.data, 0, i5 * 8);
            this.sampleDataBuffer.setPosition(0);
        }
        this.output.sampleData(this.sampleDataBuffer, i5);
        this.output.sampleMetadata(this.timeUs, 1, i5, 0, (TrackOutput.CryptoData) null);
    ...
    private void parseStreamMuxConfig(ParsableBitArray parsableBitArray) throws ParserException {
        ...
        if (readBits == 1) {
            i5 = parsableBitArray2.readBits(1);
        } else {
            i5 = 0;
        }
        this.audioMuxVersionA = i5;
        if (i5 == 0) {
            if (readBits == 1) {
                latmGetValue(parsableBitArray2);
            }
            if (parsableBitArray2.readBit()) {
                ...
                if (readBits2 == 0 && readBits3 == 0) {
                    if (readBits == 0) {
                        ...
                        parsableBitArray2.skipBits(((int) latmGetValue(parsableBitArray2)) - parseAudioSpecificConfig(parsableBitArray));
                    }
                    parseFrameLength(parsableBitArray);
                    boolean readBit2 = parsableBitArray2.readBit();
                    this.otherDataPresent = readBit2;
                    this.otherDataLenBits = 0;
                    if (readBit2) {
                        if (readBits == 1) {
                            this.otherDataLenBits = latmGetValue(parsableBitArray2);
                        } else {
                            do {
                                readBit = parsableBitArray2.readBit();
