...
public final class PackageInfoCompat {
    ...
    public static List<Signature> getSignatures(@NonNull PackageManager packageManager, @NonNull String str) throws PackageManager.NameNotFoundException {
        ...
        if (Build.VERSION.SDK_INT >= 28) {
            SigningInfo a5 = packageManager.getPackageInfo(str, 134217728).signingInfo;
            if (Api28Impl.hasMultipleSigners(a5)) {
                signatureArr = Api28Impl.getApkContentsSigners(a5);
            } else {
                signatureArr = Api28Impl.getSigningCertificateHistory(a5);
            }
        } else {
    ...
    public static boolean hasSignatures(@NonNull PackageManager packageManager, @NonNull String str, @Size(min = 1) @NonNull Map<byte[], Integer> map, boolean z4) throws PackageManager.NameNotFoundException {
        byte[][] bArr;
        if (map.isEmpty()) {
            return false;
        }
